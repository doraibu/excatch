...
int _exc_manage(const char* name, int action); // action: 0=alloc, 1=find, 2=free
void exc_throw(const char* name, int code) __attribute__((noreturn));

#define try \
    { \
        int _idx = _exc_manage(__func__, 0); \
        volatile int _err = 0; \
        asm volatile ( \
            "leaq 1f(%%rip), %%rax \n\t" \
            ...
            : "=m"(_exc_arena[_idx].rip), "=m"(_exc_arena[_idx].rsp), "=m"(_exc_arena[_idx].rbp), \
              ...
              "=m"(_exc_arena[_idx].r14), "=m"(_exc_arena[_idx].r15), "=r"(_err) \
            : : "rax", "memory" \
        ); \
        if (_err == 0)
...
void exc_throw(const char* name, int code) {
    ...
    asm volatile (
        ...
        : : "r"(_exc_arena[idx].rip), "m"(_exc_arena[idx].rbx), "m"(_exc_arena[idx].r12),
            ...
            "m"(_exc_arena[idx].rbp), "m"(_exc_arena[idx].rsp), "r"(code)
        : "rax"
    );
